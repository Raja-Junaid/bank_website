...
const Navbar = () => {
  ...
  return (
    <nav className="relative w-full flex justify-between items-center max-container">
      ...
      <ul className={`absolute top-10 right-0 bg-cyan-800 w-[140px] h-[190px] flex-1 ${toggle ? 'flex' : 'hidden'} justify-center items-center flex-col rounded-lg sidebar customShadow z-30`}>
        {navLinks.map((navlink, index) => (
          <li key={navlink.id}>
